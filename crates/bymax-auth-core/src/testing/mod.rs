...
pub struct InMemoryStores {
    ...
    recent_auth: Mutex<HashMap<(SessionKind, String), ()>>,
...
impl SessionStore for InMemoryStores {
    ...
    async fn mark_recent_auth(
        ...
        lock(&self.recent_auth).insert((kind, user_id_hash.to_owned()), ());
        Ok(())
    }
    ...
    async fn has_recent_auth(
        ...
        Ok(lock(&self.recent_auth).contains_key(&(kind, user_id_hash.to_owned())))
    }
